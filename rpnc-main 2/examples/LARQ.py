...
from mac_params import MyQueue
...
class BufferController:
  def __init__(self,content,node,framebytes,windowsize=8,arq_mode=1,dack=False,infile=None,prt_msgq=None):
    ...
    if node == 'A' or 'B':
      ...
      self.frame = mac_frame_handler.RelayFrame(framebytes)
    ...
    self.rtt_file = None
    if node=='A':
      self.rtt_file = open("RTT_Result_A_SW.dat",'w')
    else:
      self.rtt_file = open("RTT_Result_B_SW.dat",'w')
    self.rtt_file.write("\n=============================\n")
    ...
    elif self.arq_mode == L_SR_SACK_MODE:
      self.arq = L_SR_ARQ(node,windowsize,self.f,sack_flag=True)
...
class N_SW_ARQ: 
  ...
  def update(self,rec_seq,rec_ack):
    ...
    self.lock.release()
    print("SW rec_pkt_num",self.rx_npkt)
    return isNewPkt
...
class N_SR_ARQ:
  ...
  def update(self,rec_seq,rec_ack,rec_sack=None):
    ...
    self.lock.release()
    #print ("SR rec_pkt_num",self.rx_npkt)
...
if __name__ == '__main__':
  ...
  parser.add_option("-t", "--times", type="int", default=10, help="time slots")
  ...
  # add relay node R
  #nodeR = BufferController([],'R',framesize,windowsize,mode,True,f)
  ...
  for i in range(run_times):
    ...
    # add the operation of relay

      #dataR_list = string_to_hex_list(fxor[19:19+pktsize])
      #dataR = [int(dataR_num,16) for dataR_num in dataR_list]
